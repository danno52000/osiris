...
export default function Dashboard() {
  ...
  const [activeLayers, setActiveLayers] = useState({
    ...
    cf_outages: false,
    cf_attacks: false,
    /* Border & Threat Corridors dock panel — CBP waits + cartel geofences. */
    cbp_waits: false,
    plaza_geofences: false,
  });
  ...
  useEffect(() => {
    ...
    // Border & Threat Corridors — sidecar proxies (poll keeps waits fresh
    // in the interval effect below; this is just the first paint).
    if ((activeLayers as any).cbp_waits) {
      loadLayerOnce('cbp_waits', '/api/fusion/border-wait-times', d => ({ border_wait_times: d.ports || [] }));
    }
    if ((activeLayers as any).plaza_geofences) {
      loadLayerOnce('plaza_geofences', '/api/fusion/conflict-zones', d => ({ plaza_geofences: d.zones || [] }));
    }
  ...
  useEffect(() => {
    ...
    if ((activeLayers as any).cbp_waits) {
      intervals.push(setInterval(() => fetchEndpoint('/api/fusion/border-wait-times', d => ({ border_wait_times: d.ports || [] }), undefined, { skipWhenHidden: true }), 60000)); // 1m
    }
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── GIDEON title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'GIDEON'.split('').map((letter, i) => (
                <motion.span
                  key={i}
                  ...
                  style={{ color: '#00E5FF', textShadow: '0 0 30px rgba(0,229,255,0.25)' }}
                >
                  {letter}
                </motion.span>
              ))}
            ...
            <div className="overflow-hidden mb-8 z-[2]">
              ...
              >
                <p className="text-[11px] md:text-[10px] font-mono tracking-[0.5em] text-[var(--cyan-primary)]" style={{ opacity: 0.8 }}>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          ...
          <img src="/gideon-logo.png" alt="GIDEON" className="w-8 h-8 md:w-11 md:h-11 shrink-0 rounded-md mix-blend-screen drop-shadow-[0_0_10px_rgba(0,229,255,0.35)]" />
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.4em] font-mono" style={{ background: 'linear-gradient(135deg, #00E5FF, #0088CC)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>GIDEON</h1>
            <span className="text-[9px] md:text-[10px] font-mono tracking-[0.2em] opacity-80 uppercase" style={{ color: '#00E5FF' }}>CROSS-DOMAIN INTELLIGENCE PLATFORM</span>
          </div>
        </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 max-[1400px]:top-24 right-6 z-[200] pointer-events-none flex items-center gap-3 text-[10px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <span className="pointer-events-auto glass-panel px-3 py-1.5 text-[9px] font-mono tracking-widest border-[#00E676]/30 text-[#00E676]/90">
          CLASSIFICATION: UNCLASSIFIED // REL TO USA
        </span>
      </motion.div>
      ...
      {!isMobile && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-[250] pointer-events-auto bg-black/40 backdrop-blur-sm p-1 rounded-full border border-white/5">
        ...
        <div className="relative group">
          <button onClick={() => { setShowIntel(false); setShowAlerts(false); setShowMarkets(false); setShowSpaceCam(v => !v); setShowBorderIntel(false); }} className={`relative w-8 h-8 rounded-full flex items-center justify-center transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-white/50 ${showSpaceCam ? 'bg-[#00E5FF]/20' : 'hover:bg-white/10'}`} title="Live from Space — 24/7 video downlink from the ISS" aria-label="Live from Space" aria-expanded={showSpaceCam}>
        ...
        <div className="relative group">
          <button onClick={() => { setShowAlerts(!showAlerts); setShowIntel(false); setShowMarkets(false); setShowDrawing(false); setShowBorderIntel(false); }} className={`relative w-8 h-8 rounded-full flex items-center justify-center transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-white/50 ${showAlerts ? 'bg-[#FF3D3D]/20' : 'hover:bg-white/10'}`} title="Live Alerts — earthquakes, conflicts, breaking news" aria-label="Live Alerts" aria-expanded={showAlerts}>
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[10px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'GIDEON RECON' : mobilePanel === 'remote' ? 'WORLD REMOTE' : 'SEARCH'}
                    </span>
