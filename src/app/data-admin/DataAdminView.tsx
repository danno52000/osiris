...
const BADGE_STYLE: Record<LoadedBadgeTone, string> = {
  loaded: 'text-[#00E676] border-[#00E676]/40',
  not_loaded: 'text-[#FF5722] border-[#FF5722]/40',
  unknown: 'text-white/50 border-white/20',
...
function SourceCard({ source }: { source: DataAdminSource }) {
  ...
  const badge = loadedBadge(source);
  const loadedFlag = source.flags?.loaded;
  const badgeTitle = nonLive
    ? `disposition: ${source.disposition} (static manifest evidence)`
    : [
        `disposition: ${source.disposition}`,
        `live loaded: ${flagText(loadedFlag)} (${loadedFlag?.provenance ?? 'unknown'})`,
        loadedFlag?.basis ? `basis: ${loadedFlag.basis}` : null,
        loadedFlag?.observed_at ? `observed ${formatTimestamp(loadedFlag.observed_at)}` : null,
      ].filter(Boolean).join('\n');
  return (
    <article
    ...
    >
      <header className="flex flex-wrap items-start justify-between gap-2">
        ...
        <span
          ...
          data-loaded-live={badge.liveValue === null ? 'unknown' : String(badge.liveValue)}
          className={`px-2 py-0.5 rounded border text-[9px] font-mono font-bold tracking-widest ${BADGE_STYLE[badge.tone]}`}
        >
          {badge.label}
        </span>
      </header>
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-[11px]">
        ...
        {!nonLive && (
          <>
            <dt className="text-white/40">Source period (publisher)</dt>
            <dd className="text-white/80" data-field="source-period">
              {formatSourcePeriod(source.source_period)}
              {source.source_period?.note ? <span className="block text-white/40">{source.source_period.note}</span> : null}
            </dd>
            {source.source_period?.declared_coverage ? (
              <>
                <dt className="text-white/40">Declared coverage (manifest)</dt>
                <dd className="text-white/80" data-field="declared-coverage">
                  {source.source_period.declared_coverage}
                  <span className="text-white/30"> · {source.source_period.declared_coverage_provenance ?? 'evidence'}, not source freshness</span>
                </dd>
              </>
            ) : null}
            <dt className="text-white/40">Published in GIDEON (ingestion)</dt>
            <dd className="text-white/80 font-mono" data-field="published-at">{formatTimestamp(source.published_at)}</dd>
...
export function DataAdminView({ feed, resolved }: { feed: FeedState; resolved: ResolvedView }) {
  const { view, staleOrigin, retainedAgeS, body } = resolved;
    ...
    <div className="docs-root min-h-screen bg-[var(--bg-void)] text-[var(--text-primary)] antialiased">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-6">
        ...
        <section
          ...
          data-stale-origin={staleOrigin ?? undefined}
        ...
        >
          ...
          {view === 'unavailable' && (
            <span>
              {feed.fetchError
                ? `Browser fetch failed (${feed.fetchError}); no retained payload within the ${Math.round(RETAINED_MAX_AGE_S / 3600)} h stale limit.`
                : reasonText ?? 'No data available.'}{' '}
          ...
          {view === 'stale' && staleOrigin === 'server' && (
            <span>
              Showing the last good payload from {formatTimestamp(body?.checked_at)} ({body?.cache_age_s ?? '?'} s old); latest sidecar refresh failed
              {body?.refresh_error ? ` — ${describeReason(body.refresh_error)} (${body.refresh_error})` : ''}.
            </span>
          )}
          {view === 'stale' && staleOrigin === 'browser' && (
            <span>
              Browser fetch failed at {formatTimestamp(feed.fetchedAt)} ({feed.fetchError}); showing retained payload checked{' '}
              {formatTimestamp(body?.checked_at)}, received {formatTimestamp(feed.bodyAt)} ({retainedAgeS ?? '?'} s old). Not fresh.
            </span>
          )}
          ...
          {view === 'available' && (
            <span className="text-white/60">
              ...
              {feed.recovered ? <span className="text-[#00E676]"> · recovered</span> : null}
            </span>
          )}
        ...
        <footer className="text-[10px] text-white/30 border-t border-white/[0.06] pt-3">
          ...
          The LOADED badge follows the live loaded flag, not the reviewed disposition.
        </footer>
      </div>
    </div>
  );
