...
const FALLBACK_ORIGIN = 'http://157.230.228.37:3000';
      ...
      <header className="sticky top-0 z-[300] border-b border-white/[0.06] bg-[var(--bg-void)]/85 backdrop-blur-xl">
        <div className="max-w-[1600px] mx-auto px-4 md:px-8 h-14 flex items-center gap-3">
          <Link href="/" className="flex items-center gap-2.5 shrink-0 group">
            ...
            <img src="/gideon-logo.png" alt="GIDEON" className="w-6 h-6 rounded-md mix-blend-screen drop-shadow-[0_0_8px_rgba(0,229,255,0.5)] transition-all group-hover:drop-shadow-[0_0_10px_rgba(0,229,255,0.8)]" />
            <span className="flex flex-col leading-none">
              <span className="text-[12px] font-bold tracking-[0.3em] text-[var(--cyan-primary)] font-mono">
                GIDEON
              </span>
          ...
          <a
            href="https://github.com/danno52000/3BAI-fusion"
            target="_blank"
        ...
        <main ref={mainRef} className="flex-1 min-w-0 py-12 lg:py-16 max-w-3xl">
          ...
          <div className="mb-20">
            ...
            <p className="text-[15px] leading-[1.75] text-[var(--text-secondary)] max-w-[42rem]">
              GIDEON aggregates aviation, maritime, seismic, conflict, cyber, and OSINT feeds onto a single
...
#   "timestamp": "2026-07-29T12:00:00Z" }`}</Pre>
          ...
          <Section id="self-hosting" eyebrow="Guide" title="Self-Hosting">
            <p>GIDEON needs Node 20+ and no database. A local instance is three commands:</p>
            <Pre label="Local development" lang="bash">{`git clone https://github.com/danno52000/3BAI-fusion.git --recurse-submodules
cd 3BAI-fusion/osiris
npm install
...
docker compose up -d`}</Pre>
          ...
          <footer className="border-t border-white/[0.06] pt-6 pb-16 flex flex-wrap items-center gap-x-5 gap-y-2 text-[10px] font-mono text-[var(--text-muted)]">
            {[
              { href: 'https://github.com/danno52000/3BAI-fusion', label: 'GitHub' },
              { href: 'https://github.com/danno52000/3BAI-fusion/issues', label: 'Report an issue' },
            ].map(l => (
              <a
                key={l.label}
                href={l.href}
