...
function request(query = ''): Request {
  ipCounter += 1;
  return new Request(`http://osiris.test${GEO_PROXY_PATH}${query}`, { headers: { 'x-forwarded-for': `10.4.0.${ipCounter}` } });
}
function sidecar(status: number, body: unknown): Response {
  return new Response(JSON.stringify(body), { status, headers: { 'content-type': 'application/json' } });
}

describe('GET /api/fusion/dossiers/las-bambas-matarani/geography', () => {
  ...
  it('forwards the full publication verbatim, no-store, to the literal sidecar geography route', async () => {
    ...
    expect(body).toEqual(GEO_FULL);
    expect(body.publication.features).toHaveLength(3);
    expect(body.publication.links).toHaveLength(2);
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toMatch(/\/api\/v1\/foundation\/dossiers\/las-bambas-matarani\/geography$/);
  ...
  it('forwards the partial publication (Pillones missing, both links omitted) verbatim', async () => {
    ...
    expect(body.publication.missing).toHaveLength(1);
    expect(body.publication.omitted_links).toHaveLength(2);
  });
  ...
  it('malformed claim-bearing 200 -> 503 state-only sidecar_contract_invalid (nothing drawn)', async () => {
    type M = Record<string, unknown>;
    const cases: Array<(b: typeof GEO_FULL) => void> = [
      (b) => { b.publication!.features.pop(); }, // link endpoint without a visible feature
      (b) => { (b.publication!.features[0] as unknown as M).reviewer = 'x'; },
      (b) => { b.ddd!.publication_sha256 = '0'.repeat(64); },
      (b) => { b.publication!.legend = 'Routes'; },
      (b) => { b.publication!.features[1].label = 'Pillones · exact transfer facility'; },
      (b) => { (b.publication!.links[0] as unknown as M).geometry = { type: 'LineString', coordinates: [] }; b.publication!.links[0].to_entity_id = 'facility:unlocated'; },
      (b) => { (b.publication!.links[0] as unknown as M).geometry_basis = 'route_snapped'; },
      (b) => { b.publication!.links[0].reported_distance_km = 438; b.publication!.links[0].distance_basis = null; },
      (b) => { b.publication!.attribution = []; }, // ODbL anchors without attribution
      (b) => { b.publication!.features[1].source.attribution = 'OSM'; },
      (b) => { b.publication!.missing.push({ entity_id: 'asset:aiddata-site-20', reason: 'x' }); },
      (b) => { b.publication!.features[0].lat = 95; },
    ...
    for (const mutate of cases) {
      ...
      expect(JSON.stringify(body)).not.toContain('Pillones');
    }
  });
